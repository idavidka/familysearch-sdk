...
/**
 * Get preferred parent relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * const preferred = await getPreferredParentRelationship(sdk, "PPPP-PPP");
...
export async function getPreferredParentRelationship(
	...
	try {
		const response = await sdk.get<PreferredRelationshipResponse>(
			`/platform/tree/persons/${personId}/preferred-parent-relationship`
		);
		return response.data || null;
	} catch (error) {
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to get preferred parent relationship for ${personId}:`,
			error
		);
		return null;
	}
...
/**
 * Set preferred parent relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * await setPreferredParentRelationship(sdk, "PPPP-PPP", "RRRR-RRR");
...
export async function setPreferredParentRelationship(
	...
	try {
		...
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to set preferred parent relationship for ${personId}:`,
			error
		);
		throw error;
	}
...
/**
 * Delete preferred parent relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * const result = await deletePreferredParentRelationship(sdk, "PPPP-PPP");
...
export async function deletePreferredParentRelationship(
	...
	try {
		...
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to delete preferred parent relationship for ${personId}:`,
			error
		);
		throw error;
	}
...
/**
 * Get preferred spouse relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * const preferred = await getPreferredSpouseRelationship(sdk, "PPPP-PPP");
...
export async function getPreferredSpouseRelationship(
	...
	try {
		const response = await sdk.get<PreferredRelationshipResponse>(
			`/platform/tree/persons/${personId}/preferred-spouse-relationship`
		);
		return response.data || null;
	} catch (error) {
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to get preferred spouse relationship for ${personId}:`,
			error
		);
		return null;
	}
...
/**
 * Set preferred spouse relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * await setPreferredSpouseRelationship(sdk, "PPPP-PPP", "RRRR-RRR");
...
export async function setPreferredSpouseRelationship(
	...
	try {
		...
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to set preferred spouse relationship for ${personId}:`,
			error
		);
		throw error;
	}
...
/**
 * Delete preferred spouse relationship for a person
 * 
 ...
 * @example
 * ```typescript
 * const result = await deletePreferredSpouseRelationship(sdk, "PPPP-PPP");
 * console.log("Status:", result.statusCode);
 * ```
 */
export async function deletePreferredSpouseRelationship(
	...
	try {
		...
		sdk["logger"].error(
			`[FamilySearch SDK] Failed to delete preferred spouse relationship for ${personId}:`,
			error
		);
		throw error;
	}
