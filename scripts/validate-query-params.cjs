...
function extractQueryParamsFromImplementation(implFile, functionName) {
  ...
  // Find the function
  const funcRegex = new RegExp(`export async function ${functionName}[^{]+{([^}]+URLSearchParams[^}]+)}`, 's');
...
for (const [name, config] of Object.entries(IMPLEMENTATIONS)) {
  ...
  const implParams = extractQueryParamsFromImplementation(config.implementation, config.function);
  ...
  if (missingInImpl.length > 0 || extraInImpl.length > 0) {
    ...
    if (missingInImpl.length > 0) {
      console.log(`   Missing in implementation: ${missingInImpl.join(', ')}`);
    }
    if (extraInImpl.length > 0) {
      console.log(`   Extra in implementation: ${extraInImpl.join(', ')}`);
    }
    totalErrors++;
  } else {
    console.log(`\n✅ Parameters match!`);
...
console.log(`   Success rate: ${((totalChecked - totalErrors) / totalChecked * 100).toFixed(1)}%\n`);
